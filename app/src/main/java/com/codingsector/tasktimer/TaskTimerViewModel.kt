...
class TaskTimerViewModel(application: Application) : AndroidViewModel(application) {
        ...
        SharedPreferences.OnSharedPreferenceChangeListener { sharedPreferences, key ->
            when (key) {
                SETTINGS_IGNORE_LESS_THAN -> {
                    ...
                    Log.d(
                        TAG,
                        "settingListener: now ignore timings less than $ignoreLessThan seconds"
                    )
        ...
        if (task.name.isNotEmpty()) {
            ...
            if (task.id == 0L) {
                viewModelScope.launch(Dispatchers.IO) {
                    ...
                    // we need to test that uri isn't null.
                    // Because the contentResolver's insert function can return null.
                    // Although we're using kotlin we can't escape from the null checks completely,
    ...
    fun startEditing(taskId: Long) {
        if (BuildConfig.DEBUG && editedTaskId != 0L) throw IllegalStateException(
            "startEditing called without stopping previous edit. editTaskId: $editedTaskId, taskId: $taskId"
        )
        editedTaskId = taskId
    }
    ...
    private fun saveTiming(currentTiming: Timing) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            if (inserting) {
                ...
                Log.d(TAG, "saveTiming: save timing with the duration ${currentTiming.duration}")
                getApplication<Application>().contentResolver.update(
    ...
    private fun retrieveTiming(): Timing? {
        ...
        val timingCursor: Cursor? = getApplication<Application>().contentResolver.query(
            ...
            null,   // passing null for the projection returns all the columns.
            null,
            null,
            null
        )

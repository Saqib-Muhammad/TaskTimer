...
internal class AppDatabase private constructor(context: Context) : SQLiteOpenHelper(
    ...
    private fun addDurationsView(db: SQLiteDatabase) {
        /*
         CREATE TABLE vwTaskDurations AS
         SELECT Tasks.Name,
         Tasks.Description,
         Timings.StartTime,
    ...
    // the app don't save duration sorter then specified in ignorelessthan option, so to save it
    // we use the parameteriseView function
    private fun parameteriseView(db: SQLiteDatabase) {
        ...
        sSQL = """CREATE VIEW ${DurationsContract.TABLE_NAME} AS
            ...
            WHERE ${ParametersContract.TABLE_NAME}.${ParametersContract.Columns.ID} =
            ${ParametersContract.ID_SHORT_TIMING})
            GROUP BY ${TasksContract.TABLE_NAME}.${TasksContract.Columns.ID}, ${DurationsContract.Columns.START_DATE}
        ;""".replaceIndent(" ")
        Log.d(TAG, sSQL)
        db.execSQL(sSQL)
        ...
        sSQL =
            """INSERT INTO ${ParametersContract.TABLE_NAME} VALUES (${ParametersContract.ID_SHORT_TIMING}, 0);"""
        Log.d(TAG, sSQL)
        db.execSQL(sSQL)
    }

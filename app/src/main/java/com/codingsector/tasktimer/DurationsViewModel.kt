...
class DurationsViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val contentObserve = object : ContentObserver(Handler(Looper.getMainLooper())) {
    ...
    private fun applyFilter() {
        ...
        if (displayWeek) {
            // show record for the entire week
            ...
            // calculate week start and week end dates
            calendar.apply {
                set(GregorianCalendar.DAY_OF_WEEK, weekStart)
                set(GregorianCalendar.HOUR_OF_DAY, 0)   // Note: HOUR_OF_DAY, not HOUR !!!
    ...
    fun deleteRecords(timeInMilliseconds: Long) {
        // clear all record from Timings table prior to the date selected.
        ...
        val selections = "${TimingsContract.Columns.TIMING_START_TIME} < ?"
    ...
    override fun onCleared() {
        ...
        getApplication<Application>().contentResolver.unregisterContentObserver(contentObserve)
